...
void multi(int m,float *Sum,float Vec[],float Data[],int n)
{
  int i=0,j=0,k=0;
  while(i<m)
  {
...
int main(int argc,char *argv[])
{
  ...
  if(rank==0)
  {
    ...
    printf("tamaño de vector = %d\n",count);
    if(column!=count) { printf("El tamaño no coincide"); MPI_Abort(MPI_COMM_WORLD,0); }
    fseek( fp, 0, SEEK_SET );
    for(i=0;i<column;i++)
    {
      fscanf(fp,"%f",&vec[i]); 
  ...
  if(rank==0)
  {
    printf("\nMatrix Vector Multiplication es:\n");
    for(i=0;i<row;i++)
    {
       printf("%.3f\n",result[i]);
    }     
